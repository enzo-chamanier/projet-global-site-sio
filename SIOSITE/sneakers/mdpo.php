<?php 
	...
	if (isset($_POST['loginOuEmail'])) {
		...
		if ($rs_user->rowCount() == 0)
			$message_erreur = "<p style='font-size:16px;background-color:rgba(0, 0, 0, 0.5);border:1px solid white;'>Pas de correspondance trouvées ni dans les logins ni dans les emails</p>";
		else {
...
<?php
	...
	<div class=formEtape1>
		<form method=post>
			<h1 style=text-decoration:underline >Oubli du mot de passe</h1>
			<table>
				<tr><td><td colspan=2 class=messageOK>
					<?= isset($messageOK)?$messageOK:'  '; ?>
				<tr><td><input name=loginOuEmail style="width:300px;height:20px;border-radius: 15px 50px 30px;text-align:center;padding-left:10px;background-color:rgba(0, 0, 0, 0.5);color:white;border-color:white;" placeholder="LOGIN/MAIL*"
					<?= isset($loginOuEmail)?"value=$loginOuEmail":""; ?>>
				<tr><td><input type=submit id="envoyerMDPO" style="margin:0 auto; align-items:center; display:table;width:150px;height:20px;border-radius: 15px 50px 30px;text-align:center;padding-left:10px;background-color:rgba(0, 0, 0, 0.5);color:white;border-color:white;">
				<tr><td style=font-size:80% colspan=2>* : champ obligatoire
